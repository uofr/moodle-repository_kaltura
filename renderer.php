...
require_once(dirname(dirname(dirname(__FILE__))) . '/mod/kalvidres/renderable.php');
...
/**
 * Renderer class of Kaltura media resource.
...
class mod_kalvidres_renderer extends plugin_renderer_base {
     ...
     * @param object $kalvidres - object of Kaltura Media resource.
     * @return string - HTML markup to embed media.
     */
    public function embed_media($kalvidres) {
        ...
        if (!empty($entry_obj)) {
            ...
            if (KalturaMediaType::IMAGE == $entryobj->mediaType) {
                $markup = local_kaltura_create_image_markup($entryobj, $kalvidres->name, $theme,
                                                            KALTURA_IMAGE_DESKTOP_WIDTH, KALTURA_IMAGE_DESKTOP_HEIGHT);
            } else {
                $entryobj->width = $kalvidres->width;
                $entryobj->height = $kalvidres->height;

                if (0 == strcmp($theme, 'mymobile')) {
                    $markup = local_kaltura_get_kwidget_code($entryobj, $kalvidres->uiconf_id, $courseid, $session);
                } else {
                    $markup = local_kaltura_get_kdp_code($entryobj, $kalvidres->uiconf_id, $courseid, $session);
                }
            }
    ...
    public function create_access_link_markup($id) {
        ...
        if ($admin == true || $teacher == true) {
            ...
            $output .= get_string('view_access_logs', 'kalvidres');
            $output .= '</a>';
            $output .= '</p>';
        }
     ...
     * @return string - HTML markup to display link to access status page.
     */
    public function create_access_list_markup($kalvidres, $moduleid, $sort, $order, $page, $tablerows, $perpage, &$rows) {
        global $CFG, $COURSE, $OUTPUT, $DB;

        $url = new moodle_url('/mod/kalvidres/access_logs.php');
        ...
        if (!empty($kalvidres->entry_id)) {
                ...
                foreach ($studentlist as $student) {
                    ...
                    if ($i >= $page * $perpage  and $i < ($page + 1) * $perpage) {
                        ...
                        if ( $j % $tablerows == 0 ) {
                            ...
                            $output .= '<a href="' . $link . '">' . get_string('plays', 'kalvidres') . '</a>';
                            ...
                            $output .= '<a href="' . $link . '">' . get_string('views', 'kalvidres') . '</a>';
            ...
            if ($i == 0) {
                ...
                $stats .= get_string('no_student', 'kalvidres');
            ...
            if ($totalplays > 0 || $totalviews > 0) {
                ...
                $stats .= html_writer::end_tag('p');
                $stats .= html_writer::start_tag('p');
                $stats .= get_string('totalplays', 'kalvidres') . ' : ' . $totalplays .'';
                $stats .= html_writer::end_tag('p');
                $stats .= html_writer::start_tag('p');
                $stats .= get_string('totalviews', 'kalvidres') . ' : ' . $totalviews .'';
                $stats .= html_writer::end_tag('p');
            } else {
                $stats .= html_writer::start_tag('p');
                $stats .= get_string('not_viewed', 'kalvidres');
                $stats .= html_writer::end_tag('p');
            }
    ...
    public function create_export_excel_markup($id, $sort, $order) {
        ...
        $output .= $this->single_button(new moodle_url('/mod/kalvidres/export_excel.php',
     ...
     */
    public function display_kalvidresources_table($course) {
        global $CFG, $PAGE, $OUTPUT;

        echo html_writer::start_tag('center');

        if (!get_coursemodules_in_course('kalvidres', $course->id)) {
            echo get_string('noresources', 'kalvidres');
        ...
        $courseindexsummary = new kalvidres_course_index_summary($usesections, $strsectionname);

        $resourcecount = 0;

        if (!empty($modinfo) && !empty($modinfo->instances['kalvidres'])) {
            foreach ($modinfo->instances['kalvidres'] as $cm) {
                if (!$cm->uservisible) {
                    continue;
                }
        ...
        if ($resourcecount > 0) {
            $pagerenderer = $PAGE->get_renderer('mod_kalvidres');
     ...
     * @param kalvidres_course_index_summary $indexsummary - Structure for index summary.
     * @return string - HTML for assignments summary table.
     */
    public function render_kalvidres_course_index_summary(kalvidres_course_index_summary $indexsummary) {
        ...
        foreach ($indexsummary->resources as $info) {
            ...
            $link = html_writer::link(new moodle_url('/mod/kalvidres/view.php', $params), $info['cmname']);
