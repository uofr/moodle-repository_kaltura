...
$thingnode = $coursenode->add(get_string('access_logs', 'kalvidres'),
                              new moodle_url('/mod/kalvidres/access_logs.php' . '?id=' . $id));
...
if ($admin == false && $teacher == false) {
    ...
    echo '<h3>' . get_string('access_logs', 'kalvidres') . '</h3>';

    $renderer = $PAGE->get_renderer('mod_kalvidres');

    echo $renderer->create_export_excel_markup($id, $sort, $order);

    $output = $renderer->create_access_list_markup($kalvidres, $id, $sort, $order, $page, $tablerows, $perpage, $rows);

    $page = $OUTPUT->paging_bar($rows, $page, $perpage,
                                new moodle_url('/mod/kalvidres/access_logs.php',
